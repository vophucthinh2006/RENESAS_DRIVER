...
/* -----------------------------------------------------------------------
 * SCI_Clock_Init — release module stop for one SCI channel.
 * Wraps RWP unlock/lock around the MSTPCRB write.
 * ----------------------------------------------------------------------- */
 ...
 *
 * Pin mapping table (EK-RA6M5 schematic + RA6M5 pin function table):
 ...
 *   UART7 : RX=P614, TX=P613  PSEL=0x05  ← used for board UART
 ...
 *   effective clock per bit = PCLKB / 4
 ...
 * Hardware automatically clears TDRE when TDR is written; no manual clear needed.
 * ----------------------------------------------------------------------- */
void UART_SendChar(UART_t uart, char data)
{
    ...
    SCI_TDR(n) = (uint8_t)data;           /* write byte — hardware clears TDRE */
}
